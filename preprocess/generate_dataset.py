...
def GenerateMod(sourcedir, savedir, scale):
    ...
    #print(filepaths)
    #print(type(os.listdir(sourcedir)))

    filepaths = sorted(filepaths)
    #print('souredatadir',filepaths)
    num_files = len(filepaths)
    #print('fdjfkjdk',num_files)
    ...
    for filename in tqdm(filepaths):
        ...
        width = int(np.floor(image.shape[1] / mod_scale))   #shape[0]：表示矩阵的行数
        height = int(np.floor(image.shape[0] / mod_scale))  #shape[1]：表示矩阵的列数
        # modcrop
